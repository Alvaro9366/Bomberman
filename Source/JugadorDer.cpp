...
void JugadorDer::disminuirvida(int PuntaIzquierda, int PuntaDerecha, int CentroInicioY, int CentroFinalY,
	...
	if (getX() >= PuntaIzquierda && getX() <= PuntaDerecha && getY() >= CentroInicioY && getY() <= CentroFinalY) {
		x = 50*filas; //Relocalizacion del jugador si pierde una vida
		y = 50 * (columnas - 5);
		vidas--;
	}
